                     ...
                     <?php
                      require '../config/config.php';  
                           $conn=connection();
                            ...
                            $sql="select videoname,dataid from history where userid={$userid} ORDER by time desc";
                            $result=query_dml($conn,$sql);
                            $count=0;
                            //print_r($result);
                            while($row = mysqli_fetch_assoc($result)) {
                                    echo '<div class="col-md-4"><div class="jumbotron"><video src="http://trendsepc.in/sih2018/public/video/'.$row['videoname'].'"></video>
                                    <form action="video_analysis.php" method="post" align="center">
                                        <input type="text" name="name" value="'.$row['dataid'].'" hidden />
                                        <button type="submit" class="btn btn-info">'.$row['videoname'].'</button>
                                    </form>
                                    <h5></h5>
                                    </div>';
                                    echo $row['time'];
